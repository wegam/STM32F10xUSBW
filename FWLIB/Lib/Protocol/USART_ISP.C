...
void Usart_ISP_SlaveProcess(ISP_Conf_TypeDef *ISP_Conf)		//模块作为从机时的处理程序
{
	...
	RxNum=USART_ReadBufferIDLE(ISP_Conf->USARTx,(u32*)ISP_Conf->ISP_DATA.ISP_RvBuffer,(u32*)ISP_Conf->ISP_DATA.ISP_RxBuffer);	//串口空闲模式读串口接收缓冲区，如果有数据，将数据拷贝到RevBuffer,并返回接收到的数据个数，然后重新将接收缓冲区地址指向RxdBuffer
	if(RxNum)
	{
		ISP_Conf->OverRunTime=0;									//超时时间
		if(RxNum==1)		//单字节----识别波特率
		{
			if(ISP_Conf->ISP_DATA.ISP_RvBuffer[0]==0x7F)	//自举应答
			{
				Usart_ISP_Reset(ISP_Conf);														//重置编程器---恢复所有参数为默认值
				ISP_Conf->ISP_SLAVE_STATUS=ISP_STATUS_WaitCommand;		//ISP等待命令（作为从机)
				Usart_ISP_ACK(ISP_Conf);															//ISP应答
			}
		}
		else if(RxNum==2&&ISP_Conf->ISP_SLAVE_STATUS==ISP_STATUS_WaitCommand)				//2字节---接收命令过程
		{
			Usart_ISP_CommandRead(ISP_Conf);			//串口接收命令（主机->从机)
		}
		else if((RxNum==5&&(ISP_Conf->ISP_SLAVE_STATUS==ISP_STATUS_WaitRAddr||ISP_Conf->ISP_SLAVE_STATUS==ISP_STATUS_WaitWAddr))||ISP_Conf->ISP_SLAVE_STATUS==ISP_STATUS_WaitEraseAddr)				//5个字节---地址类数据接收地址过程
		{
			Usart_ISP_GetAddr(ISP_Conf);						//ISP获取写数据起始地址(主机->从机)
		}
		else if(RxNum==2&&ISP_Conf->ISP_SLAVE_STATUS==ISP_STATUS_WaitLengh)				//2字节---接收待处理数据长度，读数据时，传入地址后再传入待读取的数据长度
		{
	...
	else if(ISP_Conf->ISP_SLAVE_STATUS==ISP_STATUS_WaitRData||ISP_Conf->ISP_SLAVE_STATUS==ISP_STATUS_WaitReaded)	//等待数据读取完成
	{
		Usart_ISP_ReadMemory(ISP_Conf);														//ISP读数据（从机->主机）
	}	
	else if(ISP_Conf->ISP_SLAVE_STATUS==ISP_STATUS_WaitSData||ISP_Conf->ISP_SLAVE_STATUS==ISP_STATUS_WaitSended)		//ISP等待发送数据
	{
...
//	else if(ISP_Conf->ISP_SLAVE_STATUS==ISP_STATUS_WaitSended)	//等待数据发送完成
//	{
//		//......
//		ISP_Conf->ISP_SLAVE_STATUS=ISP_STATUS_WaitCommand;				//等待新的命令
//	}
	else	if(ISP_Conf->ISP_SLAVE_STATUS!=ISP_STATUS_IDLE)				//非初始状态时超时复位
	{
		ISP_Conf->OverRunTime=ISP_Conf->OverRunTime+1;					//超时时间
		if(ISP_Conf->OverRunTime>=100000)		//约5秒
		Usart_ISP_Reset(ISP_Conf);				//重置编程器---恢复所有参数为默认值
	}
}
/*******************************************************************************
...
void Usart_ISP_CommandRead(ISP_Conf_TypeDef *ISP_Conf)			//串口接收命令（主机->从机)
{
	...
	if(C0==ISP_COMMAND_Get)			//获取当前自举程序版本及允许使用的命令
	{
		...
		ISP_Conf->ISP_SLAVE_STATUS=ISP_STATUS_WaitCommand;		//ISP等待命令（作为从机)--处理完Get命令后等待下一下命令
		rxNum=15;			//上报数据大小
	...
	else if(C0==ISP_COMMAND_GetVS)	//获取自举程序版本及 Flash 的读保护状态
	{
		...
		ISP_Conf->ISP_SLAVE_STATUS=ISP_STATUS_WaitCommand;		//ISP等待命令（作为从机)--处理完GetVS命令后等待下一下命令
		rxNum=5;			//上报数据大小
	}
	else if(C0==ISP_COMMAND_GetID)	//获取芯片 ID
	{
		...
		ISP_Conf->ISP_SLAVE_STATUS=ISP_STATUS_WaitCommand;		//ISP等待命令（作为从机)--处理完GetID命令后等待下一下命令
		rxNum=5;			//上报数据大小
	}
	else if(C0==ISP_COMMAND_RM)			//从应用程序指定的地址开始读取最多 256 个字节的存储器空间：读数据分五步：1-读数据命令，2-读数据起始地址，3-需要读取的长度，4-读数据过程，5-上报数据
	{
		ISP_Conf->ISP_SLAVE_STATUS=ISP_STATUS_WaitRAddr;			//ISP等待待读取地址---从机接收到读数据命令后应答，然后等待待读取数据地址
		Usart_ISP_ACK(ISP_Conf);															//ISP应答
		return;
	}
	else if(C0==ISP_COMMAND_Go)			//等待待跳转到内部 Flash 或 SRAM 内的应用程序代码地址//Go 命令用于从应用程序指定的地址开始执行已下载的代码或其它任何代码
	{
		ISP_Conf->ISP_SLAVE_STATUS=ISP_STATUS_WaitGoAddr;			//接收到Go命令后，应答，然后等待开始执行起始地址
		Usart_ISP_ACK(ISP_Conf);	//ISP应答
	}
	else if(C0==ISP_COMMAND_WM)			//Write Memory:从应用程序指定的地址开始将最多 256 个字节的数据写入 RAM 或 Flash
	{
		ISP_Conf->ISP_SLAVE_STATUS=ISP_STATUS_WaitWAddr;			//ISP等待待写入地址---接收到写命令后，应答，然后等待写入起始地址
		Usart_ISP_ACK(ISP_Conf);	//ISP应答
	}
	else if(C0==ISP_COMMAND_Erase)	//擦除一个到全部 Flash 页面
	{
		ISP_Conf->ISP_SLAVE_STATUS=ISP_STATUS_WaitEraseAddr;	//ISP等待待擦除地址，接收到擦除命令后，应答，等待输入待擦除地址（全部擦除或者页擦除）
		Usart_ISP_ACK(ISP_Conf);	//ISP应答
	}
	...
	{
//		if(ISP_Conf->ISP_STEPS==Step_WaitRlen)	//等待接收需要读取的数据长度
//		{
//			if(ISP_Conf->ISP_STATUS==ISP_STATUS_WRITE)		//FLASH执行写操作
//			{
//				rxNum=ISP_Conf->ISP_RvBuffer[0];						//待读取的长度
//				ISP_Conf->ISP_TxBuffer[0]=ISP_ANSWER_ACK;		//应答位
//				memcpy(&ISP_Conf->ISP_TxBuffer[1], ISP_Conf->ISP_TvBuffer, rxNum+1);
//				rxNum+=2;
//				ISP_Conf->ISP_STEPS=Step_SendRData;		//发送读取的数据
//			}
//			else
//			{
//				rxNum=ISP_Conf->ISP_RvBuffer[0];						//待读取的长度
...
//			}
//		}
//		else if(ISP_Conf->ISP_STEPS==Step_Erase)	//擦除
//		{
//			if(ISP_Conf->ISP_RvBuffer[0]==0xFF)			//全页擦除
//			{
//				ISP_Conf->ISP_STEPS=Step_EraseAll;		//发送读取的数据
//				//.....等待擦除完成
//				Usart_ISP_ACK(ISP_Conf);	//ISP应答
//				ISP_Conf->ISP_STEPS=Step_Start;				//复位步骤
//			}
//			else
//			{
//				ISP_Conf->ISP_STEPS=Step_ErasePage;		//擦除页
//				//.....等待擦除完成
//				Usart_ISP_ACK(ISP_Conf);	//ISP应答
//				ISP_Conf->ISP_STEPS=Step_Start;				//复位步骤
//			}
...
{	
	...
	if(ISP_Conf->ISP_SLAVE_STATUS==ISP_STATUS_WaitEraseAddr)			//ISP擦除操作，接收到擦除命令后，应答，等待输入待擦除地址（全部擦除或者页擦除）等待擦除完成，完成后应答
	{
		...
		ISP_Conf->ISP_SLAVE_STATUS=ISP_STATUS_WaitErase;						//等待接收待写入的数据
	}
	else
	{
		...
		if(ISP_Conf->ISP_SLAVE_STATUS==ISP_STATUS_WaitRAddr)					//等待接收读数据起地址，接收到地址后应答，再等待待读取数据长度
		{
			ISP_Conf->ISP_DATA.ReadAddr=addr;														//待读数据的起始地址
			ISP_Conf->ISP_SLAVE_STATUS=ISP_STATUS_WaitLengh;						//等待接收需要读取的数据长度
			Usart_ISP_ACK(ISP_Conf);	//ISP应答
		}
...
//	if(ISP_Conf->ISP_STEPS==Step_WaitRaddr)		//当前状态为等待接收需要读取数据的起始地址
//	{
//		
//		
//		ISP_Conf->ReadAddr=addr;
//		ISP_Conf->ISP_SLAVE_STATUS=ISP_STATUS_WaitLengh;		//ISP等待命令（作为从机)
//		Usart_ISP_ACK(ISP_Conf);						//ISP应答
//	}
//	else if(ISP_Conf->ISP_STEPS==Step_WaitWaddr)		//等待接收需要写入的数据起始地址
//	{
//		if(ISP_Conf->ISP_STATUS==ISP_STATUS_Go)		//跳转到内部 Flash 或 SRAM 内的应用程序代码
//		{
...
//			
//			ISP_Conf->ISP_STATUS=ISP_STATUS_IDLE;	//FLASH空闲状态，可以读写
//			ISP_Conf->ISP_STEPS=Step_Start;				//起始步骤，当前为空闲
//			
//			Usart_ISP_ACK(ISP_Conf);							//ISP应答
//			
//			Usart_ISP_Reset(ISP_Conf);						//重置编程器---恢复所有参数为默认值
...
//			addr+=(ISP_Conf->ISP_RvBuffer[3]);
//			
//			ISP_Conf->WriteAddr=addr;						//存储地址
//			ISP_Conf->ISP_STEPS=Step_WaitWData;	//等待接收需要写入的数据长度
//			Usart_ISP_ACK(ISP_Conf);						//ISP应答
//		}
//	}
}
...
void Usart_ISP_GetLengh(ISP_Conf_TypeDef *ISP_Conf)				//ISP获取需要读取的数据长度(主机->从机)，读数据时，传入地址后再传入待读取的数据长度
{
	...
	{
		ISP_Conf->ISP_DATA.ReadLen=C0;										//待读取的数据长度
		ISP_Conf->ISP_SLAVE_STATUS=ISP_STATUS_WaitRData;	//ISP等待读数据操作
	}
}
/*******************************************************************************
*函数名			:	function
...
{	
	if(ISP_Conf->ISP_SLAVE_STATUS==ISP_STATUS_WaitRData)				//ISP等待读数据操作
	{
		//*******************执行读数据操作
		#ifdef	Usart_ISP_Simulation 		//模拟从机
		{
//			ISP_Conf->ISP_DATA.ISP_TxBuffer[0]=0x79;
//			memcpy(&ISP_Conf->ISP_DATA.ISP_TxBuffer[1], ISP_Conf->ISP_DATA.ISP_TvBuffer, ISP_Conf->ISP_DATA.ReadLen);	//复制数据
//			USART_DMASend(ISP_Conf->USARTx,(u32*)ISP_Conf->ISP_DATA.ISP_TxBuffer,ISP_Conf->ISP_DATA.ReadLen+2);				//串口DMA发送程序
			ISP_Conf->ISP_SLAVE_STATUS=ISP_STATUS_WaitSData;		//ISP等待发送数据			
		}
		#else
		{
			//*******************
			ISP_Conf->ISP_SLAVE_STATUS=ISP_STATUS_WaitReaded;		//ISP等待读读数完成
		}
		#endif		
	}
	else if(ISP_Conf->ISP_SLAVE_STATUS==ISP_STATUS_WaitReaded)	//ISP等待读读数完成
	{
		//*******************等待数据读取完成，数据读取完成后发送数据，然后等待发送完成
		ISP_Conf->ISP_SLAVE_STATUS=ISP_STATUS_WaitSended;		//ISP等待数据上传完成
	}
...
{	
	if(ISP_Conf->ISP_SLAVE_STATUS==ISP_STATUS_WaitSData)
	{
		...
		memcpy(&ISP_Conf->ISP_DATA.ISP_TxBuffer[1], ISP_Conf->ISP_DATA.ISP_TvBuffer, ISP_Conf->ISP_DATA.ReadLen);	//复制数据
		USART_DMASend(ISP_Conf->USARTx,(u32*)ISP_Conf->ISP_DATA.ISP_TxBuffer,ISP_Conf->ISP_DATA.ReadLen+2);				//串口DMA发送程序
		ISP_Conf->ISP_SLAVE_STATUS=ISP_STATUS_WaitSended;				//等待新的命令
	}
	...
	#ifdef	Usart_ISP_Simulation 		//模拟从机
	{
		if(ISP_Conf->ISP_SLAVE_STATUS==ISP_STATUS_WaitErase)	//开始擦除
		{
			if(ISP_Conf->ISP_DATA.WriteAddr==0xFF)	//全部擦除
			{
				...
				Usart_ISP_Reset(ISP_Conf);												//重置编程器---恢复所有参数为默认值---模式切换后复位
				ISP_Conf->ISP_SLAVE_STATUS=ISP_STATUS_WaitErased;	//状态为擦除完成
			}
		}
		else if(ISP_Conf->ISP_SLAVE_STATUS==ISP_STATUS_WaitErased)	//等待擦除完成
		{
			ISP_Conf->ISP_SLAVE_STATUS=ISP_STATUS_WaitCommand;	//状态为擦除完成，ISP等待命令（作为从机)
		}
	}
	#else
	{
...
{	
	...
	ISP_Conf->OverRunTime						=	0;								//超时时间
	ISP_Conf->ISP_DATA.NumToSave		=	0;		//接收到的有效数据
	ISP_Conf->ISP_DATA.NumToSend		=	0;		//需要往串口发送的数据个数
	ISP_Conf->ISP_DATA.OffsetAddr		=	0;		//写从机时的地址偏移
	ISP_Conf->ISP_DATA.StartAddr		=	0;		//起始地址
	ISP_Conf->ISP_DATA.ReadAddr			=	0;		//读数据起始地址
	ISP_Conf->ISP_DATA.ReadLen			=	0;		//需要读取的长度
