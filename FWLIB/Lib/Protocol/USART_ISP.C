...
void Usart_ISP_SlaveProcess(ISP_Conf_TypeDef *ISP_Conf)		//模块作为从机时的处理程序
{
	...
	if(RxNum)
	{
		...
		else if((RxNum==5&&(ISP_Conf->ISP_SLAVE_STATUS==ISP_STATUS_WaitRAddr||ISP_Conf->ISP_SLAVE_STATUS==ISP_STATUS_WaitWAddr||ISP_Conf->ISP_SLAVE_STATUS==ISP_STATUS_WaitGoAddr))||ISP_Conf->ISP_SLAVE_STATUS==ISP_STATUS_WaitEraseAddr)				//5个字节---地址类数据接收地址过程
		{
...
//	}
	else	if(ISP_Conf->ISP_SLAVE_STATUS!=ISP_STATUS_IDLE)				//非初始状态时超时复位
	{
		...
		if(ISP_Conf->OverRunTime>=2000000)		//约5秒
		Usart_ISP_Reset(ISP_Conf);				//重置编程器---恢复所有参数为默认值
	}
}
/*******************************************************************************
...
//			ISP_Conf->ISP_SLAVE_STATUS=ISP_STATUS_WaitWData;						//等待接收待写入的数据
		...
		else if(ISP_Conf->ISP_SLAVE_STATUS==ISP_STATUS_WaitGoAddr)			//等待接收写数据地址，接收到地址后应答，再等待待写入的数据
		{
			...
			ISP_Conf->ISP_SLAVE_STATUS=ISP_STATUS_IDLE;							//ISP空闲状态，可以读写
			Usart_ISP_ACK(ISP_Conf);	//ISP应答
		}
	}
}
...
void Usart_ISP_GetLengh(ISP_Conf_TypeDef *ISP_Conf)				//ISP获取需要读取的数据长度(主机->从机)，读数据时，传入地址后再传入待读取的数据长度
{
	...
	else
	{
		...
		Usart_ISP_SetSlaveStatus(ISP_Conf,ISP_STATUS_WaitRData);	//设置从机状态--ISP等待读数据操作
...
{	
	if(ISP_Conf->ISP_SLAVE_STATUS==ISP_STATUS_WaitRData)				//ISP等待读数据操作
	{
		...
		#ifdef	Usart_ISP_Simulation 		//模拟从机
		{
			...
			memcpy(&ISP_Conf->ISP_DATA.ISP_TxBuffer[1], ISP_Conf->ISP_DATA.ISP_TvBuffer, ISP_Conf->ISP_DATA.ReadLen+1);	//复制数据
...
{	
	#ifdef	Usart_ISP_Simulation 		//模拟从机
	{
		...
		{
			unsigned short Wlen=ISP_Conf->ISP_DATA.ISP_RvBuffer[0];	//获取要写入的数据长度（字节数)
			unsigned char Bcc=BCC8(ISP_Conf->ISP_DATA.ISP_RvBuffer,Wlen+2);		//异或校验;
			if(Bcc!=ISP_Conf->ISP_DATA.ISP_RvBuffer[Wlen+2])
			{
				return;
			}
			memcpy(ISP_Conf->ISP_DATA.ISP_TvBuffer, &ISP_Conf->ISP_DATA.ISP_RvBuffer[1], Wlen+1);	//复制数据
			ISP_Conf->ISP_SLAVE_STATUS=ISP_STATUS_WaitWrited;				//ISP等待写入完成
		}
...
void Usart_ISP_Erase(ISP_Conf_TypeDef *ISP_Conf)				//ISP擦除操作，接收到擦除命令后，应答，等待输入待擦除地址（全部擦除或者页擦除）等待擦除完成，完成后应答
{
	...
	#else
	{
		if(ISP_Conf->ISP_SLAVE_STATUS==ISP_STATUS_WaitErase)	//开始擦除
		{
			...
			{
				Usart_ISP_ACK(ISP_Conf);							//ISP应答---擦除完成
				Usart_ISP_SetSlaveStatus(ISP_Conf,ISP_STATUS_WaitErased);	//设置从机状态--状态为擦除完成
//				ISP_Conf->ISP_SLAVE_STATUS=ISP_STATUS_WaitErased;	//状态为擦除完成
			}
		}
